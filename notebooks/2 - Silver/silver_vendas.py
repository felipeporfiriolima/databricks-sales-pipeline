...
from config import CATALOGO, BRONZE, SILVER, VENDA
...
def vw_vendas():
    return (
        spark.readStream
        .table(tabela_bronze)
        .filter(col("_rescued_data").isNull() | (col("flg_valida") == 1))
        .select("id_venda","data_venda","id_cliente","id_loja","id_status","id_produto","quantidade","vl_unitario","dtCarga")
        .drop("_rescued_data")
        .drop("arquivo_origem")
        .drop("flg_valida")
    )
