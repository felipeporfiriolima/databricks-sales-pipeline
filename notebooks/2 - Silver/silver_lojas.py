...
from config import CATALOGO, BRONZE, SILVER, LOJA
...
view = "vw_lojas_tratadas"

@dp.view(name=view)
def vw_lojas_tratadas():
    return (
        spark.readStream
        .table(tabela_bronze)
        .withColumn("estado",upper(col("estado")))
        .withColumn("dtAtualizacao",current_timestamp())
        .withColumn("id_loja",col("id_loja").cast("int"))
        .filter(col("_rescued_data").isNull())
        .filter(col("id_loja").isNotNull())
        .filter(col("nome").isNotNull())
        .select("id_loja","nome","estado","dtCarga","dtAtualizacao")
        .drop("_rescued_data")
        .drop("arquivo_origem")
    )
