...
from config import CATALOGO, BRONZE, CLIENTE, CLIENTE_QUARENTENA, SCHEMA_CLIENTE, DATASET

catalogo = CATALOGO
schema = BRONZE
tabela = CLIENTE
quarentena = CLIENTE_QUARENTENA
tabela_bronze = f"{catalogo}.{schema}.{tabela}"
tabela_quarentena = f"{catalogo}.{schema}.{quarentena}"
schema = SCHEMA_CLIENTE
arquivo = f"{DATASET}/{CLIENTE}/"
