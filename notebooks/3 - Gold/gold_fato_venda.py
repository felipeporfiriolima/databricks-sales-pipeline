...
from config import CATALOGO, SILVER, GOLD, VENDA, VENDA_GOLD
...
def Fato_venda():
    return (
        ...
        .table(tabela_silver)
        .select(
                col("id_venda").alias("id_venda"),
                col("data_venda").alias("dt_venda"),
                col("id_cliente").alias("id_cliente"),
                col("id_loja").alias("id_loja"),
                col("id_status").alias("id_status"),
                col("id_produto").alias("id_produto"),
                col("quantidade").alias("nr_qtd"),
                col("vl_unitario").alias("vl_unitario")
        )
        .withColumn(("vl_total"), col("nr_qtd") * col("vl_unitario"))
    )